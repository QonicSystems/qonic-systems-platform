...
export function TestimonialCarousel() {
  ...
  return <div className="testimonial-region" role="region" aria-roledescription="carousel" aria-label="Client testimonials" onKeyDown={(event) => { if (event.key === "ArrowLeft") previous(); if (event.key === "ArrowRight") next(); }}>
    <article className="testimonial-card" aria-live="polite"><div className="testimonial-stars" aria-label="5 out of 5 stars">★★★★★</div><blockquote>“{testimonials[current].quote}”</blockquote><div className="mt-8 flex items-center gap-4"><span className="avatar">{testimonials[current].initials}</span><div><strong>{testimonials[current].name}</strong><p>{testimonials[current].role}</p></div></div></article>
